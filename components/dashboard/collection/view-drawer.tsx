...
export function ViewDrawer<T extends Record<string, any>>({ item, children, editUrl, viewUrl }: ViewDrawerProps<T>) {
  ...
  return (
    <Drawer direction={isMobile ? "bottom" : "right"}>
      ...
      <DrawerContent className={isMobile ? "" : "sm:max-w-md ml-auto h-full rounded-l-xl"}>
        <div className="flex h-full flex-col">
          <DrawerHeader className="gap-1 border-b pb-4">
            <div className="flex items-center gap-2 mb-1">
              {isPost && <FileText className="size-4 text-blue-500" />}
              {isMedia && <ImageIcon className="size-4 text-purple-500" />}
              {(isCategory || isTag) && <Tag className="size-4 text-emerald-500" />}
            ...
            <DrawerTitle className="leading-tight text-xl font-bold">{title}</DrawerTitle>
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-6 scrollbar-hide">
            ...
            <div className="grid gap-6">
              ...
              {(item.excerpt || item.description) && (
                <section className="bg-muted/30 p-4 rounded-xl border border-dashed border-muted-foreground/20">
                  <h4 className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground/60 mb-2 flex items-center gap-1.5">
                    <FileText className="size-3" />
                    Summary / Description
                  </h4>
                  <p className="leading-relaxed text-sm text-foreground/80 italic">
              ...
              {isPost && (
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <DetailBox label="Nano ID" icon={<div className="size-1.5 rounded-full bg-muted-foreground" />}>
                    <span className="font-mono text-[9px] uppercase tracking-tighter">{item.nid}</span>
                  </DetailBox>
                </div>
              )}
              ...
              {/* Timestamps */}
              <div className="flex items-center justify-between text-[10px] text-muted-foreground/60 pt-4 border-t">
...
function DetailBox({ label, icon, children }: { label: string, icon: React.ReactNode, children: React.ReactNode }) {
  return (
    <div className="flex flex-col gap-1.5 p-3 rounded-xl border bg-background shadow-[0_1px_2px_rgba(0,0,0,0.05)]">
       <span className="text-[9px] font-bold uppercase tracking-widest text-muted-foreground/50 flex items-center gap-1">
        {icon}
        {label}
       </span>
       <div className="text-sm font-semibold truncate leading-none py-0.5">
        {children}
       </div>
    </div>
  )
